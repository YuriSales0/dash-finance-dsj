-- 020: P&L pega novas categorias customizadas automaticamente
-- cost_other agora e um catch-all: pega cost_office, uncategorized negativos,
-- E qualquer NOVA categoria de custo que nao esteja nas colunas especificas
...
BEGIN
    ...
    SELECT
        -- Revenue: TODA categoria do grupo revenue (incluindo customs) + uncategorized positivo
        COALESCE(SUM(CASE
            WHEN c.group_name = 'revenue' THEN t.amount_usd
            WHEN t.category_id IS NULL AND t.amount_usd > 0 THEN t.amount_usd
            ELSE 0
        END), 0),
        -- Custos especificos (colunas dedicadas)
...
-- Regenerar P&L dos ultimos 12 meses com a nova logica
DO $$
DECLARE
  m DATE;
BEGIN
