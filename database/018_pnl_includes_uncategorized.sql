-- 018: Incluir transacoes sem categoria no P&L
-- Antes: JOIN categories excluia transacoes uncategorized
-- Depois: LEFT JOIN, conta uncategorized como cost_other/revenue_other
...
BEGIN
    ...
    -- Agregar por categoria (incluindo uncategorized como cost_office/revenue_other)
    SELECT
        -- Revenue: categoria revenue + uncategorized positivo
        COALESCE(SUM(CASE
            WHEN c.group_name = 'revenue' THEN t.amount_usd
        ...
        COALESCE(SUM(CASE WHEN t.category_id = 'cost_refunds' THEN ABS(t.amount_usd) ELSE 0 END), 0),
        ...
        -- cost_other: categoria cost_office + uncategorized negativo
        COALESCE(SUM(CASE
            WHEN t.category_id = 'cost_office' THEN ABS(t.amount_usd)
...
-- Apos atualizar a funcao, regenerar os P&Ls dos ultimos 12 meses
DO $$
DECLARE
  m DATE;
BEGIN
